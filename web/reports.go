...
func (web *Web) rankingsPdfReportHandler(w http.ResponseWriter, r *http.Request) {
	...
	colWidths := map[string]float64{"Rank": 13, "Team": 22, "RP": 18, "Coop": 18, "Match": 22, "Auto": 18, "Stage": 18,
		"W-L-T": 23, "DQ": 18, "Played": 18}
	rowHeight := 6.5
	...
	for _, ranking := range rankings {
		...
		pdf.CellFormat(colWidths["Coop"], rowHeight, strconv.Itoa(ranking.CoopertitionBonus), "1", 0, "C", false, 0, "")
...
func (web *Web) schedulePdfReportHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, match := range matches {
		...
		if breakIndex < len(scheduledBreaks) && scheduledBreaks[breakIndex].TypeOrderBefore == match.TypeOrder {
			...
			pdf.CellFormat(colWidths["Match"]+4*colWidths["Team"], rowHeight, description, "1", 1, "C", false, 0, "")
			breakIndex++
		}
		...
		if surrogate {
			...
			pdf.CellFormat(colWidths["Team"], height, surrogateText(match.Blue1IsSurrogate), "LBR", 0, "CT", false, 0,
				"")
			pdf.CellFormat(colWidths["Team"], height, surrogateText(match.Blue2IsSurrogate), "LBR", 1, "CT", false, 0,
				"")
			pdf.SetFont("Arial", "", 10)
		}
	}
